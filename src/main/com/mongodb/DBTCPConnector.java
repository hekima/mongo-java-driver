...
public class DBTCPConnector
     ...
     * Start a "request".
     * 
     * A "request" is a group of operations in which order matters. Examples include inserting a document and
     * then performing a query which expects that document to have been inserted, or performing an operation
     * and then using com.mongodb.Mongo.getLastError to perform error-checking on that operation. When a
     * thread performs operations in a "request", all operations will be performed on the same socket, so they
     ...
     * 
     * By ending a request when it is safe to do so the built-in connection- pool is allowed to reassign
     * requests to different sockets in order to more effectively balance load. See requestStart for more
     * information.
     */
    @Override
    ...
    void _checkClosed()
    {
        if (_closed.get()) throw new IllegalStateException("this Mongo has been closed");
    }
    ...
    private WriteResult innerSay(DB db, OutMessage m, WriteConcern concern, ServerAddress hostNeeded)
    {
        ...
        if (concern == null) { throw new IllegalArgumentException("Write concern is null"); }
        ...
        catch (IOException ioe)
        {
            ...
            if (concern.raiseNetworkErrors()) throw new MongoException.Network("Write operation to server " +
                                                                               port.host() + " failed on database " +
                                                                               db, ioe);
    ...
    private Response innerCall(final DB db, final DBCollection coll, final OutMessage m,
                               final ServerAddress hostNeeded, final int retries, ReadPreference readPref,
                               final DBDecoder decoder)
    {
        if (readPref == null) readPref = ReadPreference.primary();

        if (readPref == ReadPreference.primary() && m.hasOption(Bytes.QUERYOPTION_SLAVEOK)) readPref = ReadPreference.secondaryPreferred();
        ...
        try
        {
            ...
            if (res._responseTo != m.getId()) throw new MongoException("ids don't match");
        }
        catch (IOException ioe)
        {
            _myPort.error(port, ioe);
            retry = retries > 0 && !coll._name.equals("$cmd") && !(ioe instanceof SocketTimeoutException) &&
                    _error(ioe, secondaryOk);
            if (!retry) { throw new MongoException.Network("Read operation to server " + port.host() +
                                                           " failed on database " + db, ioe); }
        }
        catch (RuntimeException re)
        {
            _myPort.error(port, re);
        ...
        if (retry) return innerCall(db, coll, m, hostNeeded, retries - 1, readPref, decoder);
        ...
        if (err != null && err.isNotMasterError())
        {
            ...
            if (retries <= 0) { throw new MongoException("not talking to master and retries used up"); }
    ...
    {
        if (_connectionStatus != null) { return _connectionStatus.getServerAddressList(); }
    ...
    boolean isMongosConnection()
    {
        ...
        else if (_connectionStatus instanceof DynamicConnectionStatus) { return ((DynamicConnectionStatus) _connectionStatus).asMongosStatus() != null; }
    ...
    /**
     * This method is called in case of an IOException. It will potentially trigger a checkMaster() to check
    ...
    class MyPort {
        ...
        DBPort get(boolean keep, ReadPreference readPref, ServerAddress hostNeeded)
        {
            ...
            if (hostNeeded != null)
            {
                if (pinnedRequestPort != null && pinnedRequestPort.serverAddress().equals(hostNeeded)) { return pinnedRequestPort; }
            ...
            else
            {
                ...
                if (node == null) throw new MongoException("No replica set members available in " + replicaSet +
                                                           " for " + readPref.toDBObject().toString());
        ...
        {
            if (!threadHasPinnedRequest()) return;

            if (getPinnedRequestPortForThread() != null) return;
        ...
        void requestDone()
        {
            ...
            if (current != null)
            {
                ...
                {
                    pinnedRequestStatusThreadLocal.remove();
                    if (current.requestPort != null) current.requestPort.getPool().done(current.requestPort);
                }
            }
        }
    ...
    void checkMaster(boolean force, boolean failIfNoMaster)
    {
        ...
        if (_connectionStatus != null)
        {
            if (_masterPortPool == null || force)
            {
                ...
                if (master == null)
                {
                    if (failIfNoMaster) throw new MongoException("can't find a master");
                }
                else
                {
                    setMaster(master);
        ...
        else
        {
            ...
            if (_maxBsonObjectSize == 0) initDirectConnection();
        }
    }
    ...
    {
        if (_masterPortPool == null) return;
    ...
    private synchronized boolean setMasterAddress(ServerAddress addr)
    {
        ...
        if (newPool == _masterPortPool) return false;

        if (_masterPortPool != null) _logger.log(Level.WARNING, "Primary switching from " +
    ...
    /**
     * Assigns a new DBPortPool for a given ServerAddress. This is used to obtain a new pool when the resolved
     * IP of a host changes, for example. User application should not have to call this method directly.
     * 
     * @param addr
     */
    ...
    /**
     * Gets the maximum size for a BSON object supported by the current master server. Note that this value
     * may change over time depending on which server is master.
     * 
